...
void PID_compute(){
    ...
    actualSpeed_two = (float)((Motor_one.motor_pulse_count / shaft_ppr) * 60 * (1000 / pid_interval));
    Motor_one.motor_pulse_count = 0;

    // compute motor one speed, unit = revolutions per min
    actualSpeed_one = (float)((Motor_two.motor_pulse_count / shaft_ppr) * 60 * (1000 / pid_interval));
