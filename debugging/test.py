...
#                     # Introduce a delay to throttle signal emission
#                     self.msleep(10)  # Adjust the delay as needed
#         finally:
#             self.ser.close()
...
#         self.serial_thread = SerialThread("/dev/cu.usbserial-10", 9600)
...
#         parts = data.split(',')
#         for part in parts:
#             # Check if ':' is in the part to ensure it can be split into key and value
#             if ':' in part:
#                 try:
...
#                     continue
#             # else:
#                 # Handle parts without ':' here. For now, just print a warning.
#                 # print(f"Warning: Data '{part}' does not conform to 'key: value' format.")
...
#                 self.lines[key], = self.ax.plot([], [], label=key)  # Set label for the line
...
#         # Include legend in the plot (only once)
...
#             if hasattr(self.serial_thread, 'ser') and self.serial_thread.ser.is_open:
